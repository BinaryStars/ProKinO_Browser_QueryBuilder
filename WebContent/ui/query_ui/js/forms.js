                    ...
                    if (metaphor == "dd") {
                        $('#' + h_id)
                            .append($("<option></option>")
                                ...
                                .attr("value", '?' + (plc.toLowerCase().replace(/ /g, '')))
                                .text('?' + (plc.toLowerCase().replace(/ /g, ''))));
                    } else {

                        $('#' + h_id).val('?' + (plc.toLowerCase().replace(/ /g, '')));
                    }
                    result_vars.push('?' + (plc.toLowerCase().replace(/ /g, '')));
                ...
                if ($('#' + query_count_id).hasClass("checkmark")) { //deselect
                    ...
                    if (metaphor == "dd") {
                        $('#' + h_id)
                            .append($("<option></option>")
                                .attr("selected", "selected")
                                .attr("value", "cnt ?" + (plc.toLowerCase().replace(/ /g, '')))
                                .text("cnt ?" + (plc.toLowerCase().replace(/ /g, ''))));
                    } else {
                        $('#' + h_id).val("cnt ?" + (plc.toLowerCase().replace(/ /g, '')));
                    }
                    count_vars.push('?' + (plc.toLowerCase().replace(/ /g, '')));
                }
...
*/
        ...
        $(document).on('click', '#' + button_id, function() {
            ...
            var jsn = $("#" + form_id).serializeObject();
            inst_to_jsn[obj_value.title.toLowerCase()] = jsn;
            form_array[form_id] = inst_to_jsn;
            //alert(JSON.stringify((form_array[form_id])));
            var inst_name = obj_value.title.toLowerCase();
            var inst_triple = "";
            ...
            $.each(jsn, function(prop, val) {
                ...
                if (isAbstract === "true") {
                    ...
                    if (val !== '') {
                        ...
                        if (val !== plch) {
                            if (val.indexOf("?") === -1) {
                                var temp_val = '?' + plch.toLowerCase().replace(/ /g, '');
                                ///
